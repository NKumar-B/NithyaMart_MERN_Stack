// Small fetch-based API client for the Lulu Mart Bangalore backend with robust seed fallback.
...
function getSeedProducts() {
  ...
  const cakes = CAKES.map(([name, serves, file, desc, price], i) => ({
    _id: `cake_${i + 1}`,
    name,
...
export const api = {
  getProducts: (params = {}) => {
    ...
    return fetch(`${API_URL}/products${qs ? `?${qs}` : ''}`)
      ...
      .then((res) => {
        ...
        if (list.length === 0) throw new Error('Empty API result');
        return list;
      })
      .catch(() => {
  ...
  getProduct: (id) => 
    fetch(`${API_URL}/products/${id}`)
      ...
      .catch(() => getSeedProducts().find(p => p._id === id || String(p.id) === String(id))),
  createOrder: (order) =>
    fetch(`${API_URL}/orders`, {
      ...
      .catch(() => ({ success: true, orderId: 'ORD-' + Math.floor(100000 + Math.random() * 900000) })),
  payOrder: (id, paymentMethod) =>
    fetch(`${API_URL}/orders/${id}/pay`, {
      ...
      .catch(() => ({ success: true, status: 'PAID' })),
}
